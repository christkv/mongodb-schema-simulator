    ...
    Scenario.prototype.execute = function(options) {
      ...
      return new Promise(function(resolve, reject) {
        co(function*() {
          ...
          yield cat.addLocal(self.words[localNumber].trim(), self.words[wordNumber].trim(), options);

          // Get end time of the cart
          var endTime = microtime.now();
          yield services.log('second', 'multilanguage_add_new_local', {
              start: startTime
            , end: endTime
    ...
    Scenario.prototype.execute = function(options, callback) {
      ...
      return new Promise(function(resolve, reject) {
        co(function*() {
          ...
          // Get end time of the cart
          var endTime = microtime.now();
          yield services.log('second', 'multilanguage_remove_local', {
              start: startTime
            , end: endTime
            , time: endTime - startTime
          });

...
Category.findAllDirectChildCategories = function(collections, path, options, callback) {
  ...
  if(coveredIndex) {
    ...
    return collections['categories'].find({parent: regexp})
...
Category.findOne = function(collections, path, options, callback) {
  ...
  if(coveredIndex) {
    ...
    return collections['categories'].findOne({category: path}
      , {fields:{_id: 0, name: 1, category:1, parent:1}} , function(err, doc) {
        if(err) return callback(err);
...
Category.createOptimalIndexes = function(collections, callback) {
  ...
  // collections['categories'].ensureIndex({category:1, name: 1, parent:1}, function(err, result) {
  //   if(err) return callback(err);

  //   collections['categories'].ensureIndex({parent:1, name: 1, category:1}, function(err, result) {
  //     if(err) return callback(err);
  //     callback();
  //   });
  // });

...
Inventory.prototype.reserve = function(id, quantity, options, callback) {
  ...
  this.inventories.updateOne({
    ...
    if(err) return callback(err);
    if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('could not add the reservation for %s with the quantity %s', id, quantity)));
    if(r.result.writeConcernError) return callback(r.result.writeConcernError);
    callback(null, self);
  });
}
...
Inventory.prototype.adjust = function(id, quantity, delta, options, callback) {
  ...
  self.inventories.updateOne({
    ...
    if(err) return callback(err);
    if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('could not adjust the reservation for %s with the change of quantity %s', id, delta)));
    if(r.result.writeConcernError) return callback(r.result.writeConcernError);
    callback(null, self);
  });
}
...
Inventory.prototype.release = function(id, options, callback) {
  ...
  self.inventories.findOne({
    ...
    self.inventories.updateOne({
      ...
      if(err) return callback(err);
      if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('failed to remove reservation for %s from inventory for product %s', id, self.id)));
      if(r.result.writeConcernError) return callback(r.result.writeConcernError);
      callback(null, self);
    });
  });
...
Inventory.commit = function(collections, id, options, callback) {
  ...
  collections['inventories'].updateMany({
    ...
    if(err) return callback(err);
    if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('no reservations for cart %s found in inventory', id)));
    if(r.result.writeConcernError) return callback(r.result.writeConcernError);
    callback();
  });
}

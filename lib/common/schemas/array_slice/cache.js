...
SliceCache.prototype.create = function(object, callback) {
  ...
  this.cache.insertOne({
    ...
    self.cache.updateOne({
      ...
      if(err) return callback(err);
      if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('failed to clear out pre-allocated array for object %s', self.id)));
      callback(null, self);
    });
  });
}
...
SliceCache.prototype.push = function(items, position, options, callback) {
  ...
  this.cache.updateOne({
    ...
    if(err) return callback(err);
    if(r.modifiedCount == 0 && r.n == 0) return callback(new Error(f('failed to push items to cache object with id %s', self.id)));
    callback(null, self);
  });
}

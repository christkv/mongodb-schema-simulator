  ...
  execute(agents) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Create instances to calculate the total ops expected
        self.manager.list().forEach(function(x) {
          objects[x.name] = {sim: x, instance: x.create()};
        });

        // Total ops
        var totalOps = 0;

        // Based on the simulations calculate the total ops
        for(var sim of self.simulation) {
          if(objects[sim.name]) {
            // Get the object
            var obj = objects[sim.name];

            // We have an execute method so calculate the total ops
            if(obj.instance.execute) {
              // Number of iterations
              var iterations = sim.execution.iterations;
              var numberOfUsers = sim.execution.numberOfUsers;

              // The total number of ops
              totalOps += (numberOfUsers * iterations);
            }
          }
        }

        // The number of processes we are running
        totalOps *= (self.number / self.simulation.length);

        // Emit the total number of ops
        self.emit('status', {ops: totalOps});
...
class EndPoint {
  ...
  start() {
    var self = this;
    var left = self.number * self.monitor.simulation.length;

  ...
  execute(agents) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Calculate the total number of operations we are performing
        // var totalOps = self.agents.length * 
        var totalOps = 0;

        // Add total ops for all simulations
        for(var s of self.simulation) {
          totalOps += s.execution.iterations * s.execution.numberOfUsers;
        }

        // Calculate total operations
        totalOps *= self.agents.length;

        // Emit the data
        self.emit('setup', totalOps);
      ...
      co(function*() {
        ...
        self.server = dnode({
          ...
          log: function(measurements, callback) {
            ...
            self.monitor.emit('log', measurements);
          },
...
//     url: 'mongodb://localhost:27017/load?maxPoolSize=50'
//   });
//   // Wait for done event

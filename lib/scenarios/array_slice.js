...
scenarios.push({
  ...
  , create: function(services, scenario, schema) {
    ...
    var createIndexes = function(db, collectionNames, callback) {
      // Get collections
      var cache = db.collection(collectionNames.cache || 'cache');

      // Create any indexes
      Cache.createOptimalIndexes(cache, function(err) {
    ...
    CacheScenario.prototype.globalSetup = function(options, callback) {      
      ...
      MongoClient.connect(scenario.url, function(err, db) {
        ...
        // Get the collections
        var cache = db.collection(collections.accounts || 'cache');
        ...
        createIndexes(db, collections, function(err) {
          ...
          for(var i = 0; i < numberOfCacheObjects; i++) {
            var account = new Cache(cache, i, initialCacheSize);            
    ...
    CacheScenario.prototype.execute = function(options, callback) {
      ...
      // Get collections
      var cache = db.collection(collections.cache || 'cache');
      ...
      // Get a cache object
      var object = new Cache(cache, id, initialCacheSize);

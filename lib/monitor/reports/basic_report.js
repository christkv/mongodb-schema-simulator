...
BasicReport.prototype.execute = function(callback) {
  ...
  // All scenario based data feeds
  var scenarioFeeds = [];
  var topologyFeeds = [];

  // Iterate over all the items
  for(var filename in report) {
    if(report[filename].type == 'scenario') {
      scenarioFeeds.push({filename: filename, scenario: report[filename]});
    } else if(report[filename].type == 'topology') {
      topologyFeeds.push({filename: filename, config: report[filename]});
    }
  }

  // Render scenarios
  renderScenarios(self, scenarioFeeds, function(err, scenarioHtml) {
...
var renderScenarios = function(self, scenarios, callback) {
  ...
  scenarios.forEach(function(x) {
    ...
    var data = JSON.parse(fs.readFileSync(x.filename, 'utf8'));
    // Split out the parts of the data
    var host = data.host;
    var port = data.port;
    // Get the schemas for the data
    if(!schemas) schemas = data.schemas.schemas;
    // Merge the data together
    mergeLogEntries(logEntries, data.logEntries);
  });

  // For each schema create a statistics collector
  // for(name in schemas) {
  schemas.forEach(function(s) { 
    var name = s.schema.name;
    statistics[name] = new RunningStats();

    // Run over all the data and calculate the statistics
    for(timestamp in logEntries[name]) {
      logEntries[name][timestamp].forEach(function(x) {
        statistics[name].push(x.time);
      });
    }
  });

  console.dir(Object.keys(logEntries))

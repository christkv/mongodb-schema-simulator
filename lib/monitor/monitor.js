...
var generateOperations = function(self, name, data, callback) {
  ...
  var graph = new gnuplot.Line({debug:true});
  graph.terminal('png');
  graph.output(f('%s/%s.png', self.argv.o, name))
  graph.xlabel('seconds');
  graph.ylabel('ops');
  graph.title(f('%s', name));
  ...
  var lines = [];
  // Add the data
  lines.push(f('"-" using 1:2 title "%s"', name));
  // Set the plot commands
  graph.plotData(lines);
  graph.execute(callback);
}
...
var generateServerOperations = function(self, name, data, callback) {
  ...
  for(var j = 0; j < fields.length; j++) {
    ...
    for(var k = 0; k < keys.length; k++) {
      entries.push(data[keys[k]][0][name]);
    }

 ...
 */
var Process = function(argv, manager) {
...
Process.prototype.start = function(callback) {
  ...
  if(this.argv.r) return callback(null, {totalExecutions:totalExecutions});

  // Start the server monitor
  this.serverProcess.start(function(err) {
    if(err) return callback(err);

    // If we are running local processes, kick it off
...
Process.prototype.error = function(errs) {
  var self = this;
  var finalErrors = {};
...
Process.prototype.stop = function(signal, callback) {
  ...
  // No children return
  if(this.children.length == 0) return callback();

...
var monitorReplicaSet = function(self, db, callback) {
  ...
  var monitorServers = function(servers) {
    ...
    self.interval = setInterval(function() {
      ...
      var executeOnServers = function(_server, callback) {
        // Execute command
        _server.command({serverStatus:true}, function(err, r) {
          if(!err) {
            // Create second based timetamp
            var timestamp = new Date();
            timestamp.setMilliseconds(0)
      ...
      for(var i = 0; i < servers.length; i++) {
        executeOnServers(servers[i]);
      }
    }, self.resolution);
  }

  // Locate all candidate servers for monitoring in set
  var locateMonitoringServers = function(callback) {
    // Execute command to get calibration record
  ...
  locateMonitoringServers(function(err) {
    if(err) return callback(err);
...
ServerMonitor.prototype.start = function(callback) {  
  ...
  MongoClient.connect(this.url, function(err, db) {
    ...
    if(db.serverConfig instanceof Server || db.serverConfig instanceof Mongos) {

    ...
    for(var i = 0; i < r.hosts.length; i++) {
      ...
      d.open(function(err, d1) {
        ...
        d1.command({ismaster: true}, function(err, r) {
          ...
          // Total servers left to check
          if(totalLeft == 0) {
            callback(errors.length > 0 ? errors : null, r);
          }
        });
      });
    }
  ...
  var monitorServers = function(servers, ismaster) {
    ...
    self.interval = setInterval(function() {
      ...
      for(var i = 0; i < servers.length; i++) {
        executeOnServers(self, ismaster.setName, servers[i]);
      }
    }, self.resolution);
  }

  // Get the candidate servers
  locateMonitoringServers(db, servers, errors, function(err, ismaster) {
    if(err) return callback(err);
          ...
          if(left == 0) {
            ...
            self.interval = setInterval(function() {
              ...
              var executeShard = function(shardInfo) {
                ...
                // Iterate over all the servers
                for(var i = 0; i < servers.length; i++) {
                  executeOnServers(self, _id, servers[i]);
                }
              }
...
inherits(ServerMonitor, EventEmitter);

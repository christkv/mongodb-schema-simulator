...
class Services {
  ...
  tick() {
    this.monitor.tick([], this.agent, function() {});
  }
...
var executeScenario = function(agent, scenario, callback) {
  ...
  console.log("====================== executeScenario :: " + (numberOfIterationsLeft) + " :: " + process.pid)
  ...
  if(scenario.instance.custom) {
    ...
    co(function*() {
      ...
      // Flush the ops cache
      agent.services.flush({noTick:true});
      // Callback
      callback();
    });
  ...
  var intervalId = setInterval(function() {
    ...
    if(totalNumberOfUsers >= 1) {
      numberOfIterationsLeft = numberOfIterationsLeft - totalNumberOfUsers;
      // // Perform a tick
      // agent.services.tick();
  ...
  //   console.log("=========================== numberOfIterationsLeft :: " + numberOfIterationsLeft)
  //   // Number of iterations are done
  //   if(numberOfIterationsLeft == 0 && dead ==false && promises.length == 0) {
  //     if(agent.logger.isDebug()) agent.logger.debug(f('[AGENT-%s:%s-%s] - done', agent.host, agent.port, process.pid));
  //     dead = true;

    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        if(self.db.serverConfig instanceof Server) {
          ...
          self.monitor = new ReplicasetServerMonitor(self.db, self.interval);
 ...
 */
class ReplicasetServerMonitor extends EventEmitter {
  constructor(db, interval) {
    super();
    this.db = db;
    this.interval = interval;
  ...
  return new Promise(function(resolve, reject) {
    co(function*() {
      ...
      for(var i = 0; i < r.hosts.length; i++) {
        ...
        var d = new Db(db.databaseName, new Server(parts[0], parts[1], {size:1}));
        var d1 = yield d.open();
